...
export class LogonTimeEditorComponent implements AfterViewInit, OnDestroy {
    ...
    setDayBit(dayId: number | null, hour: number, value: number | null = null) {
        ...
        const index = 24 * dayId + hour;
        this.bitValues.set(index, value == null 
            ? this.bitValues.get(index) == 1? 0 : 1
            : value);
    ...
    onSelectStarted($event: MouseEvent, day: LogonMapDay, hour: number) {
        ...
        fromEvent(document, 'mouseup').pipe(take(1)).subscribe(x => {
           this.finishSelection();
        })
    }
    ...
    isInsideSelection(day: LogonMapDay, hour: number) {
        if(day.id == null || day.id < 0 || this.selectDayCurrent < 0 || this.selectDayStarted < 0 
            || this.selectHourCurrent < 0 || this.selectHourStarted < 0) {
            return false;
        }
        const dayId = day.id ?? -1;
        return dayId >= Math.min(this.selectDayCurrent, this.selectDayStarted) && 
    ...
    isSelectionRowStart(day: LogonMapDay, hour: number) {
        ...
        return day.id == Math.min(this.selectDayCurrent, this.selectDayStarted) &&
                hour >= Math.min(this.selectHourCurrent, this.selectHourStarted) &&
                hour <= Math.max(this.selectHourCurrent, this.selectHourStarted);
    }
    isSelectionColumnStart(day: LogonMapDay, hour: number) {
        ...
        const dayId = day.id;
        return hour == Math.min(this.selectHourCurrent, this.selectHourStarted) &&
    ...
    isSelectionColumnEnd(day: LogonMapDay, hour: number) {
        ...
        const dayId = day.id;
        return hour == Math.max(this.selectHourCurrent, this.selectHourStarted) &&
                dayId >= Math.min(this.selectDayCurrent, this.selectDayStarted) &&
                dayId <= Math.max(this.selectDayCurrent, this.selectDayStarted);
    }
    isSelectionRowEnd(day: LogonMapDay, hour: number) {
        ...
        return day.id == Math.max(this.selectDayCurrent, this.selectDayStarted)  &&
                hour >= Math.min(this.selectHourCurrent, this.selectHourStarted) &&
                hour <= Math.max(this.selectHourCurrent, this.selectHourStarted);
    }
